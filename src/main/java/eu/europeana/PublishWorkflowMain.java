...
import eu.europeana.tools.PublishWorkflow;
 ...
 * @since 2016-07-15
 */
public class PublishWorkflowMain {
    private static final Logger logger = LogManager.getLogger();
    public static void main (String[] args) throws Exception {
        logger.info("Publish workflow");
        ...
        try(CassandraConnector cassandraConnector = CassandraConnector.getInstance(); Session session = cassandraConnector.getSession()) {
            for (int i = 0; i < runTimes; i++) {
                ...
                PublishWorkflow.publishDataset(session, provider, dataset, schema, revision, fetchSize, rowsThreshold, batch);
                long stopTime = System.currentTimeMillis();
                long elapsedTime = stopTime - startTime;
                totalRunsElapsedTime += elapsedTime;
                logger.info("Run: " + (i + 1) + ", Publish provider: " + provider + " dataset: " + dataset + " schema: " + schema
                        + " in total time: " + elapsedTime + "ms");
